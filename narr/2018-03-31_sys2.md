...
### Purpose & Remarks <a name="rem"></a>
- **Reference data** generated with feature complete s370_perf
- will be used as base line for the comparison of other systems
- a very high statistics of 92 runs was taken, allows to study the
  timing distribution with high significance.

### Findings <a name="find"></a>
- **General**
  - the test reproducibility is very good, the 50% width is around 1%.
...
- **Instruction timing**
  ...
  - `MVCIN` is very slow, a factor 6 slower than `MVN` or `MVZ`,
  ...
  - `CS`,`CDS` and `TS` slow in _lock missed_ case for multi-CPU setups,
...
The tests [T90x](../doc/s370_perf.md#user-content-tests-t90x),
[T92x](../doc/s370_perf.md#user-content-tests-t92x) and
[T95x](../doc/s370_perf.md#user-content-tests-t95x) allow to tests whether
the instruction times are additive, or in other words, whether the time for
a sequence of instructions is the sum of the measured instruction times.
...
The [T90x](../doc/s370_perf.md#user-content-tests-t90x) contain a sequences
...
The [T92x](../doc/s370_perf.md#user-content-tests-t92x) contain a sequences
...
s370_perf has several tests addressing unaligned memory access, which can
be summarized as
```
...
The [T15x tests](../doc/s370_perf.md#user-content-tests-mvc) test `MVC` over
...
`MVCIN` exhibits very poor performance. The inverse copy is naturally slower
than `MVC`, which can use highly optimizes copy routines, because it must be
handled _byte-by-byte_. `MVCIN` is much slower than the at least equally
...
The [T17x tests](../doc/s370_perf.md#user-content-tests-mvcl) test `MVCL` over
...
faster than a sequence of `MVC`.

#### CLC performance <a name="find-clc"></a>
The [T27x tests](../doc/s370_perf.md#user-content-tests-clc) test `CLC` over
...
The [T28x tests](../doc/s370_perf.md#user-content-tests-clcl) test `CLCL` over
a wide range of conditions. The
...
Decimal arithmetic is tested for 10 and 30 digit numbers. The timing for
...
The instructions `CS`, `CDS` and `TS` require interlocked memory access on
a multi-CPU system. They are typically used to implement
